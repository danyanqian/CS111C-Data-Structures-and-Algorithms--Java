...
public class AList<T extends Comparable<? super T>> implements ListInterface<T>, Comparable<AList<T>> {
	...
	public int removeAll(T element) {
		...
		// create an int array of index to remove
		int numOfElementsRemoved = 0;
		AList<Integer> indexArray = new AList<>();

		// loop through the list to find index of matching element, add to the indexArray
		for (int i = 1; i <= numberOfEntries; i++) {
			T currentItemOnList = list[i];
			if (currentItemOnList.equals(element)) { // currentItemOnList is equal to entry
				indexArray.add(Integer.valueOf(i));
				numOfElementsRemoved++;
			}
		}

		// remove from list (from back to front to avoid index change during removal)
		for (int i = numOfElementsRemoved; i>=1; i--) {
			int indexToRemove = indexArray.getEntry(i);
	...
	public boolean equals(Object other) {
		...
		if(other instanceof AList<?>) {
			...
			// loop through each item to check equality
			boolean isEqual = true;
			for (int i=1; i <= numberOfEntries; i++){
				if (otherList.getEntry(i).equals(getEntry(i)) == false){
					isEqual = false;
				}
			}
			return isEqual;
	...
	public int compareTo(AList<T> otherList) {
		...
		int sizeComparison = Integer.compare(this.numberOfEntries, otherList.numberOfEntries);
		if (sizeComparison == 0){
			int elementComparison = 0;
			for (int i=1; i <= this.numberOfEntries; i++){
				if (otherList.getEntry(i).equals(getEntry(i)) == false){
					elementComparison = this.getEntry(i).compareTo(otherList.getEntry(i));
					break;
				}
			}
